...
串行执行：
- step29_phase8.py (Retrieval usability metrics + report export)
- step30_phase8.py (Usability report diff / regression gate)
"""
...
def print_summary(results: list[RunResult], total_elapsed_s: float) -> None:
    print("========== Phase 8 Regression Summary ==========")
...
def main() -> None:
    ...
    scripts = [
        current_dir / "step29_phase8.py",
        current_dir / "step30_phase8.py",
    ]
    ...
    if all(item.success for item in results):
        print("\n[DONE] Phase 8 regression passed")
        sys.exit(0)

    print("\n[FAIL] Phase 8 regression failed")
    sys.exit(1)
