...
串行执行：
- step26_phase7.py (Recall weight sweep)
- step27_phase7.py (Sweep report diff / regression gate)
"""
...
def print_summary(results: list[RunResult], total_elapsed_s: float) -> None:
    print("========== Phase 7 Regression Summary ==========")
...
def main() -> None:
    ...
    scripts = [
        current_dir / "step26_phase7.py",
        current_dir / "step27_phase7.py",
    ]
    ...
    if all(item.success for item in results):
        print("\n[DONE] Phase 7 regression passed")
        sys.exit(0)
