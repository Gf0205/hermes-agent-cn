...
串行执行：
- step32_phase9.py (Aggregate recall logs -> report)
- step33_phase9.py (Report diff / regression gate)
"""
...
def print_summary(results: list[RunResult], total_elapsed_s: float) -> None:
    print("========== Phase 9 Regression Summary ==========")
...
def main() -> None:
    ...
    scripts = [
        current_dir / "step32_phase9.py",
        current_dir / "step33_phase9.py",
    ]
    ...
    if all(item.success for item in results):
        print("\n[DONE] Phase 9 regression passed")
        sys.exit(0)
